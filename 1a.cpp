...
public:
    ...
    void set_key(const uint8_t key[16]) {
        uint32_t K[4];
        for (int i = 0; i < 4; ++i) {
            K[i] = ntohl(*reinterpret_cast<const uint32_t*>(key + i * 4));
        }
    ...
    void encrypt(const uint8_t in[16], uint8_t out[16]) {
        uint32_t X[36];
        for (int i = 0; i < 4; ++i) {
            X[i] = ntohl(*reinterpret_cast<const uint32_t*>(in + i * 4));
        }
        ...
        for (int i = 35; i >= 32; --i) {
            uint32_t val = htonl(X[i]);
            memcpy(out + (35 - i) * 4, &val, 4);
        }
    }
